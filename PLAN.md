...
- **Patrón de políticas** (todas `TO authenticated`; `anon` no tiene NADA): `clients` → select `has_client_access(id) OR is_coordinator_or_admin()` (el coordinador ve el roster), insert **staff activo** (migración 0018: el creador no-admin queda auto-asignado como entrenador del cliente vía trigger definer), update `has_client_access`, delete admin. Tablas hijas de cliente (medidas, fotos, historia, dieta) → `has_client_access(client_id)` para select/insert/update, delete solo admin (o autor de su propio registro en medidas/fotos) — el coordinador NO las ve sin asignación. `trainer_client_assignments` → escritura `is_coordinator_or_admin()`. `schedules`/`sessions` → `is_coordinator_or_admin() OR trainer_id = auth.uid()` (cada entrenador gestiona sus propios slots; el coordinador planifica los de todos); `schedule_participants`/`session_participants` → derivadas del padre: el entrenador dueño agrega solo clientes con `has_client_access`, el coordinador/admin agrega clientes asignados al entrenador de la sesión (validación app + check DB); `attendance_records` → derivada de la sesión/participante. Catálogos → select todos, escritura admin. `payments`: select `has_client_access`; **insert `is_admin() OR (has_client_access(client_id) AND recorded_by = auth.uid())`**; update/delete solo admin. Trigger `protect_profile_privileged_cols` impide que un no-admin cambie `role`/`is_active` de ningún profile.
- **Matriz de roles delimitada (2026-09-02, migración 0018)** — flujo de sesiones: el **entrenador define sus sesiones de trabajo del día** (ad-hoc o materializando su plantilla) y él o el coordinador determinan qué clientes participan en cada una; la **duración viene determinada** por el tipo de sesión (o 60 min) y **solo el coordinador/admin la fijan o cambian** (triggers `lock_schedule_duration`/`lock_session_duration`; exentos el contexto de servicio y las sesiones generadas desde plantilla, que heredan la del slot).

  | Capacidad | Entrenador | Coordinador | Admin |
  |---|---|---|---|
  | Registrar clientes (queda auto-asignado si no es admin) | ✅ | ✅ | ✅ |
  | Ver/editar datos de entrenamiento (medidas, fotos, historia, dieta) | solo asignados | solo asignados (o se auto-asigna) | todos |
  | Registrar pagos | de sus asignados | de sus asignados | todos |
  | Editar/eliminar pagos, catálogos, planes | ❌ | ❌ | ✅ |
  | Crear sus sesiones/horarios del día y elegir participantes | ✅ (solo suyos, clientes asignados) | ✅ (de cualquier entrenador) | ✅ |
  | Marcar asistencia por participante | ✅ (sus sesiones) | ✅ | ✅ |
  | Fijar/editar la **duración** de sesiones y horarios | ❌ (la da el tipo) | ✅ | ✅ |
  | Asignar clientes ↔ entrenadores | ❌ | ✅ (incl. a sí mismo) | ✅ |
  | Crear/desactivar cuentas, promover coordinador, cambiar roles | ❌ | ❌ | ✅ |
